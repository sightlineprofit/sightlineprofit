...
export function GrowthRoadmapPanel({
  ...
  embedded?: boolean;
  initialSubTab?: "hiring" | "revenue" | "planning";
  ...
  if (isLoading) {
    if (embedded) {
      return <p className="font-sans text-sm text-muted-lt">Loading roadmap…</p>;
    }
    return (
  ...
  const inner = (
    <>
      <Tabs defaultValue={initialSubTab ?? "hiring"} className={embedded ? "" : "mt-4"}>
        <TabsList className="bg-cream/40">
          <TabsTrigger value="hiring">Hiring & growth</TabsTrigger>
          <TabsTrigger value="revenue">Revenue trajectory</TabsTrigger>
          {planningSlot ? (
      ...
      <Section eyebrow="Section 02" title="Scenario Comparison">
        ...
        {planningSlot ? (
          <TabsContent value="planning" className="mt-6">
            <p className="mb-4 font-sans text-[13px] leading-relaxed text-ch/70">
              Quick models for fees, volume, hires, bonuses, and raises — separate from the
              multi-year revenue trajectory above. Nothing saves until you act on it elsewhere.
            </p>
            {planningSlot}
          </TabsContent>
        ) : null}
